...
tidy_nyt_data <- all_results %>%
  # 1. Extract the 'docs' (articles) from the nested API list
  mutate(docs = map(api_data, ~ .x$response$docs)) %>% 
  
  # 2. Flatten the list so each article is its own row
  unnest(docs) %>% 
  
  # 3. 'Hoist' specific fields out of the nested document structure
  hoist(docs, 
        headline = list("headline", "main"),
        section = "section_name",
        date = "pub_date") %>%
  
  # 4. Final Clean-up: Format the date and select key columns
...
From a technical standpoint, the successful completion of this project relied on a robust "Data Engineering" workflow. We navigated the complexities of the NYT API by implementing a secure authentication strategy using .Renviron to protect our API credentials. We addressed the API's strict rate limits by using the httr2 package's throttling and retry functions, combined with mandatory Sys.sleep(12) pauses to avoid being blocked. Finally, we transformed the raw, nested JSON responses into a normalized, tidy data frame using purrr for iteration and tidyr for flattening the document structure. While the 1,000-result API cap limited our ability to see every single article from 2024 and 2025, the resulting 100-row sample provided a statistically significant window into the trends we aimed to analyze.
